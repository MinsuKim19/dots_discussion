...
public class PathUtils
{
    ...
    public static int RetracePortals(NavMeshQuery query, int startIndex, int endIndex
        , PolygonIDContainer path, int n, Vector3 termPos
        ...
        , int maxStraightPath)
    {
        ...
        {
            var type1 = query.GetPolygonType(path.Pool.Value.IDs[k]);
            var type2 = query.GetPolygonType(path.Pool.Value.IDs[k + 1]);
            if (type1 != type2)
            {
                Vector3 l, r;
                var status = query.GetPortalPoints(path.Pool.Value.IDs[k], path.Pool.Value.IDs[k + 1], out l, out r);
                ...
                straightPath[n] = query.CreateLocation(cpa1, path.Pool.Value.IDs[k + 1]);
        ...
        straightPath[n] = query.CreateLocation(termPos, path.Pool.Value.IDs[endIndex]);
        straightPathFlags[n] = query.GetPolygonType(path.Pool.Value.IDs[endIndex]) == NavMeshPolyTypes.OffMeshConnection ? StraightPathFlags.OffMeshConnection : 0;
        return ++n;
    }

    public static PathQueryStatus FindStraightPath(NavMeshQuery query, Vector3 startPos, Vector3 endPos
        , PolygonIDContainer path, int pathSize
        , ref NativeArray<NavMeshLocation> straightPath
        , ref NativeArray<StraightPathFlags> straightPathFlags
        ...
        , int maxStraightPath)
    {
        ...
        if (pathSize > 1)
        {
            var startPolyWorldToLocal = query.PolygonWorldToLocalMatrix(path.Pool.Value.IDs[0]);
            ...
            {
                var polyWorldToLocal = query.PolygonWorldToLocalMatrix(path.Pool.Value.IDs[apexIndex]);
                ...
                {
                    var success = query.GetPortalPoints(path.Pool.Value.IDs[i - 1], path.Pool.Value.IDs[i], out vl, out vr);
                    if (!success)
                    {
                        return PathQueryStatus.Failure; // | PathQueryStatus.InvalidParam;
                    }
                ...
                if (Perp2D(left, vr) < 0)
                {
                    var polyLocalToWorld = query.PolygonLocalToWorldMatrix(path.Pool.Value.IDs[apexIndex]);
